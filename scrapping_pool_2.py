...
products = list(db.products_ulta_other.find({ "fetch_status": 0 }))
...
#                 print("writing to json file writer --- " + json.dumps(reviews))
        ...
        try:
            reviews_inserts_result = db.reviews_ulta_other.insert_many(mongo_reviews)
...
#                    print('all comments written successfully onto db')
                product_update_result = db.products_ulta_other.update_one(
                    { "_id": product["_id"] }, 
                    { 
                        "$set": { "fetch_status": 1 }, 
